...
int usbd_ep_open(const struct usbd_endpoint_cfg *ep_cfg)
{
    ...
    if (USB_EP_DIR_IS_OUT(ep_cfg->ep_addr)) {
        ...
        g_dwc2_udc.out_ep[ep_idx].ep_type = ep_cfg->ep_transfer_type;
        ...
        USB_OTG_OUTEP(ep_idx)->DOEPCTL |= (ep_mps & USB_OTG_DOEPCTL_MPSIZ) |
                                          ((uint32_t)ep_cfg->ep_transfer_type << 18) |
        ...
        g_dwc2_udc.in_ep[ep_idx].ep_type = ep_cfg->ep_transfer_type;
        ...
        USB_OTG_INEP(ep_idx)->DIEPCTL |= (ep_cfg->ep_mps & USB_OTG_DIEPCTL_MPSIZ) |
                                         ((uint32_t)ep_cfg->ep_transfer_type << 18) | (ep_idx << 22) |
                                         USB_OTG_DIEPCTL_SD0PID_SEVNFRM |
            ...
            if (((temp & USB_OTG_GRXSTSP_PKTSTS) >> USB_OTG_GRXSTSP_PKTSTS_Pos) == STS_DATA_UPDT) {
                if (g_dwc2_udc.read_len != 0U) {
                    if (epnum == 0) {
                        ...
                        usbd_notify_callback(USB_EVENT_EP_OUT_NOTIFY, (void *)(epnum | USB_EP_DIR_OUT));
                    }
                }
                ...
                if ((ep_intr & 0x1U) != 0U) {
                    ...
                    if ((epint & USB_OTG_DIEPINT_XFRC) == USB_OTG_DIEPINT_XFRC) {
                        if (epnum == 0) {
                            ...
                            usbd_notify_callback(USB_EVENT_EP_IN_NOTIFY, (void *)(epnum | USB_EP_DIR_IN));
                        }
                    }
                }
                ep_intr >>= 1U;
