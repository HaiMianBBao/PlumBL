...
#ifndef USB_BASE
#define USB_BASE (0x40000000UL+0x6800UL) /*!< USB_IP Peripheral Registers base address */
#endif
...
int usbd_ep_open(const struct usbd_endpoint_cfg *ep_cfg)
{
    ...
    /* initialize Endpoint */
    switch (ep_cfg->ep_transfer_type) {
    ...
    if (USB_EP_DIR_IS_OUT(ep_cfg->ep_addr)) {
        ...
        g_fsdev_udc.out_ep[ep_idx].ep_type = ep_cfg->ep_transfer_type;
        ...
        g_fsdev_udc.in_ep[ep_idx].ep_type = ep_cfg->ep_transfer_type;
        ...
        if (ep_cfg->ep_transfer_type != USB_ENDPOINT_TYPE_ISOCHRONOUS) {
        ...
        while ((USB->ISTR & USB_ISTR_CTR) != 0U) {
            ...
            if (epindex == 0U) {
                ...
                if ((wEPVal & USB_EP_CTR_RX) != 0U) {
                    ...
                    usbd_notify_callback(USB_EVENT_EP_OUT_NOTIFY, (void *)(epindex & 0x7f));
                }

                if ((wEPVal & USB_EP_CTR_TX) != 0U) {
                    /* clear int flag */
                    PCD_CLEAR_TX_EP_CTR(USB, epindex);
                    usbd_notify_callback(USB_EVENT_EP_IN_NOTIFY, (void *)(epindex | 0x80));
                }
            }
        }
    }
