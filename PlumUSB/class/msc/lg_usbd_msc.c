...
#include "lg_usbd_msc.h"
#include "lg_usb_scsi.h"
#include <stdio.h>
...
static void usbd_msc_bot_abort(void)
{
    if ((usbd_msc_cfg.cbw.bmFlags == 0) && (usbd_msc_cfg.cbw.dDataLength != 0)) {
        usbd_ep_set_stall(mass_ep_data[MSD_OUT_EP_IDX].addr);
    }
    usbd_ep_set_stall(mass_ep_data[MSD_IN_EP_IDX].addr);
}
...
static void sendCSW(uint8_t CSW_Status)
{
    ...
    if (usbd_ep_write(mass_ep_data[MSD_IN_EP_IDX].addr, (uint8_t *)&usbd_msc_cfg.csw,
...
static void sendLastData(uint8_t *buffer, uint8_t size)
{
    ...
    if (usbd_ep_write(mass_ep_data[MSD_IN_EP_IDX].addr, buffer, size, NULL) != 0) {
...
static bool SCSI_processRead(void)
{
    ...
    usbd_ep_write(mass_ep_data[MSD_IN_EP_IDX].addr,
...
static bool SCSI_processWrite()
{
    ...
    usbd_ep_read(mass_ep_data[MSD_OUT_EP_IDX].addr, &usbd_msc_cfg.block_buffer[usbd_msc_cfg.scsi_blk_addr % usbd_msc_cfg.scsi_blk_size], MASS_STORAGE_BULK_EP_MPS,
                 &bytes_read);
...
static bool SCSI_CBWDecode()
{
    ...
    usbd_ep_read(mass_ep_data[MSD_OUT_EP_IDX].addr, (uint8_t *)&usbd_msc_cfg.cbw, USB_SIZEOF_MSC_CBW,
                 &bytes_read);
...
void usbd_msc_class_init(uint8_t out_ep, uint8_t in_ep)
{
    ...
    mass_ep_data[0].addr = out_ep;
    mass_ep_data[0].ep_cb = mass_storage_bulk_out;
    mass_ep_data[1].addr = in_ep;
